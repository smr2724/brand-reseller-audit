...
export function makeSellerCache(supabase: SupabaseClient<any, any, any>) {
  return {
    async read(
      ids: string[],
    ): Promise<Record<string, { name: string | null; fetched_at: string }>> {
      const out: Record<string, { name: string | null; fetched_at: string }> = {};
      if (!ids.length) return out;
      try {
        const { data, error } = await supabase
          .from("keepa_seller_cache")
          .select("seller_id, seller_name, fetched_at")
          .in("seller_id", ids);
        if (error) return out;
        for (const row of (data as CachedRow[] | null) ?? []) {
          out[row.seller_id] = { name: row.seller_name, fetched_at: row.fetched_at };
        }
      } catch {
