 ...
 *      full_name match. If one exists with a non-null email, return
 *      it as `state='already'`.
 *   2. If a row exists in `discovered` state, run the same enrich
 *      chain as the per-contact route on it.
 *   3. Otherwise insert a new `discovered` row from the candidate
 *      data, then run the enrich chain.
 *
 * The chain is `enrichSingleContact` — same path Apollo → Hunter-
 * finder → 8-pattern → LLM web-search (with MV gating at every
 * step) that the bulk endpoint uses.
 */
...
export async function POST(
  ...
  // 1. Look up existing brand_contacts row by case-insensitive full_name.
  ...
  const existing = (existingList ?? []).find(
    (r) =>
      ...
      rawName.toLowerCase(),
  ) as { id: string; full_name: string; email: string | null; enrichment_state: string } | undefined;

  // Already-enriched short-circuit.
  if (existing && existing.email && existing.enrichment_state === "enriched") {
  ...
  if (existing) {
    ...
    if (insErr || !ins) {
      return NextResponse.json(
        { error: `seed_failed: ${insErr?.message ?? "no row inserted"}` },
        { status: 500 },
      );
    }
    contactId = ins.id;
  ...
  // 3. Claim discovered → enriching atomically. Same idempotency
  // contract as the per-contact route (see route.ts comment).
